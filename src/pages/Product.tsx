...
// 🧠 Fetch data from FastAPI backend (MongoDB)
useEffect(() => {
 ...
 const handleSwipe = (e: any, { offset, velocity }: any) => {
  const swipe = offset.x * velocity.x;
  if (swipe < -1000 && currentPage === "productlist") {
    setDirection(1);
    setCurrentPage("product");
  } else if (swipe > 1000 && currentPage === "product") {
    setDirection(-1);
    setCurrentPage("productlist");
  }
};
 ...
 useEffect(() => {
  const handleKeyDown = (e: KeyboardEvent) => {
    if (e.key === "ArrowRight" && currentPage === "productlist") {
      setDirection(1);
      setCurrentPage("product");
    } else if (e.key === "ArrowLeft" && currentPage === "product") {
      setDirection(-1);
      setCurrentPage("productlist");
    }
  };
